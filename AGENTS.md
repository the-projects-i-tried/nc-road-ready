# Maintainer / coding-agent instructions
...
Do not silently add cookies, storage, analytics, service workers, API calls, model credentials, or a framework. Do not claim unlimited distinct questions when the authored pool is finite. A request can select a new variant of a familiar rule; answer shuffling is not new content. A narrow pool should explicitly report exhaustion.

Use DOM text rendering, never inject pack text as HTML. Retain source URL and asset validation. Test mobile widths, deferred-feedback check mode, imports, exports, reset, duplicate prevention, and keyboard input when editing the interface. Disclose untested browsers or hosted deployment rather than claiming comprehensive validation.

Do not create, publish, rename, or delete a remote repository unless the user explicitly asks. This bundle was prepared for the owner to push themselves.
